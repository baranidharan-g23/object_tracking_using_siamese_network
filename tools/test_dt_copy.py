...
def process_frame(bounding_box_storage, initial_template, target_pos, target_sz, siammask, cfg, device):
    if len(bounding_box_storage) >= 10:
...
def calculate_direction(past_positions):
    # Convert positions to numpy array for vector operations
    ...
    # Normalize the vector to maintain direction information
...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    ...
    # Remove the oldest position and append the current position to the list
    past_positions.pop(0)
    past_positions.append(target_pos.copy())  # Append a copy of the current position
    direction_vector = calculate_direction(past_positions)
     # Calculate the magnitude of the direction vector
    direction_magnitude = np.linalg.norm(direction_vector)
    # Define the weight_factor to adjust the influence of the direction
    # Make weight_factor adaptive based on direction magnitude
    weight_factor = min(1.0, direction_magnitude * 0.1)
    target_pos += direction_vector * weight_factor  # Adjust target position by the direction vector
    # Update the state dictionary with the new past positions list
